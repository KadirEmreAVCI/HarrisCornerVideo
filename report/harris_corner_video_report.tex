...
Because the Gaussian smoothing stage is the dominant kernel-level hotspot, a focused Nsight Compute comparison was performed for \texttt{ApplyGaussianSmoothingKernel}. The experiment used only the first frame of the 1080p input video so that both variants were evaluated on the same image content and frame resolution. A $5\times5$ Gaussian filter was selected instead of the default $3\times3$ filter to amplify the effect of neighborhood reuse: each output pixel samples 25 input values, making redundant global-memory reads more visible than in the smaller 9-sample case.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{screenshots/nsight-compute-comparison-1080p-5x5.png}
...
Compute throughput stays close to \textbf{80\%} for both runs, so the key difference is the interaction between memory access and computation rather than a large change in arithmetic utilization. For the first 1080p frame with a $5\times5$ filter and $16\times16$ CUDA blocks, shared-memory tiling provides a clear performance benefit for \texttt{ApplyGaussianSmoothingKernel}. Future work should still test whether the same advantage holds for the $3\times3$ filter, other resolutions, alternative block sizes, and separable Gaussian implementations.
